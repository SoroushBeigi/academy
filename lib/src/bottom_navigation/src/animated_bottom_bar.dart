...
import 'bottom_active_item.dart';
import 'bottom_controller.dart';
import 'bottom_inactive_item.dart';
import 'bottom_navigation_painter.dart';
...
class _AnimatedNotchBottomBarState extends State<AnimatedBottomBar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    widget.bottomController.addListener(() {
  ...
  Widget build(BuildContext context) {
    ...
    /// uncomment
    if (widget.bottomController.index > widget.bottomBarItems.length - 1) {
    ...
    return widget.bottomBarItems.length > maxCount
      ...
      child: AnimatedBuilder(
        ...
        builder: (BuildContext _, Widget? __) {
          ...
          double scrollPosition = widget.bottomController.index.toDouble();
          int? currentIndex = widget.bottomController.index;
          if (widget.bottomController.oldIndex != null) {
            _isInitial = false;
            scrollPosition = Tween<double>(
                begin: widget.bottomController.oldIndex!.toDouble(),
                end: widget.bottomController.index.toDouble())
            // ignore: invalid_use_of_protected_member
                .lerp(_animationController.value);
            currentIndex = widget.bottomController.index;
          } else {
            scrollPosition = widget.bottomController.index.toDouble();
            currentIndex = widget.bottomController.index;
          }
