...
class _NewContentFormWidgetState extends State<NewContentFormWidget> {
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.file_upload,
                size: 42, color: Theme.of(context).colorScheme.tertiary),
            SizedBox(height: 8.0),
            Text(label,
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Space.h8,
            for (int i = 0;
                i < context.read<NewContentCubit>().attachmentResults.length;
                i++)
              Stack(
                    ...
                    child: Column(
                      children: [
                        _buildFilePicker(
                          ...
                          fileResult:
                              context.read<NewContentCubit>().attachmentResults[i],
                        ),
                        Space.h8,
                        ACElevatedButton(
                          width: AppSize.s120,
  ...
  Widget _buildTagsInput() {
    ...
    return TextFormField(
      ...
      onChanged: (value) {
        setState(() {
          context.read<NewContentCubit>().tags =
              value.split(',').map((tag) => tag.trim()).toList();
        });
      },
      validator: (value) {
        if (context.read<NewContentCubit>().tags.isEmpty) {
  ...
  Future<void> _pickMainFile() async {
    ...
    context.read<NewContentCubit>().mainFileResult = await FilePicker.platform
        .pickFiles(type: FileType.custom, allowedExtensions: ['mp4']);
    print(
        'Main file picked: ${context.read<NewContentCubit>().mainFileResult?.files.single.name}');
    setState(() {});
  }
  ...
  Future<void> _pickImageFile() async {
    ...
    print(
        'Image file picked: ${context.read<NewContentCubit>().imageFileResult?.files.single.name}');
    setState(() {});
  }
  ...
  Future<void> _pickAttachment(int index) async {
    ...
    if (result != null) {
      setState(() {
        if (index >= context.read<NewContentCubit>().attachmentResults.length) {
          context.read<NewContentCubit>().attachmentResults.add(result);
        } else {
          context.read<NewContentCubit>().attachmentResults[index] = result;
        }
  ...
  void _addAttachment() {
    ...
    setState(() {
      context.read<NewContentCubit>().attachmentResults.add(null);
    });
  }
  ...
  void _removeAttachment(int index) {
    ...
    setState(() {
      context.read<NewContentCubit>().attachmentResults.removeAt(index);
    });
  }

  void _submitForm() {
    print('Submitting form...');
    if (context.read<NewContentCubit>().formKey.currentState?.validate() ??
        false) {
      context.read<NewContentCubit>().formKey.currentState?.save();
      for(var i in selectedCategoryList) {
        context.read<NewContentCubit>().categoryIds.add(i.id ?? 1);
      }
      for(var i in approvedContentsList) {
        context.read<NewContentCubit>().relatedIds.add(i.id ?? 1);
        print(i.id);
      }
      print(context.read<NewContentCubit>().categoryIds);
